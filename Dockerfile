...
RUN apt-get update -y
RUN apt-get install -y acestream-engine vlc-nox python-gevent python-setuptools python-m2crypto python-apsw

RUN mkdir -p /var/run/sshd
RUN mkdir -p /var/log/supervisor
RUN mkdir -p /tmp/acestream
RUN wget "http://dl.acestream.org/linux/acestream_3.1.16_ubuntu_16.04_x86_64.tar.gz"
RUN tar zxvf acestream_3.1.16_ubuntu_16.04_x86_64.tar.gz -C /tmp/acestream
RUN mkdir -p /home/tv/acestream
RUN mv /tmp/acestream/acestream_3.1.16_ubuntu_16.04_x86_64 /home/tv/acestream
RUN adduser --disabled-password --gecos "" tv

RUN cd /tmp/ && wget https://github.com/AndreyPavlenko/aceproxy/archive/master.zip -O master.zip
