...
if [[ -n "$TTV_URL" ]]; then
    echo "Gived link: $TTV_URL"
    sed -i "s@url=''@url=\"$TTV_URL\"@" /home/tv/aceproxy-master/plugins/config/torrenttv.py
...
sed -i 's^acestreamengine --client-console^/home/tv/acestreamengine --client-console --live-cache-type memory --live-mem-cache-size 209715200 --upload-limit 0 --live-buffer 60^' /home/tv/aceproxy-master/aceconfig.py
